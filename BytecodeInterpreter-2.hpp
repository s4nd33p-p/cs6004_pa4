...
obj:
	...
	allocateObject(REGISTER_ARGS_LIST, J9Class *clazz, bool initializeSlots = true, bool memoryBarrier = true, bool skipUnflattenedFlattenableCheck = false)
	{
		...
		//printf("Object allocation\n"); always printing this line 6 times
	...
	{
		j9object_t instance = NULL;
		//printf("Inside Indexable Object ALlocation\n");
#if defined(J9VM_OPT_VALHALLA_VALUE_TYPES)
...
recursiveOOM:
	...
	{
		//printf("Inside inlInternalsNewInstanceImpl\n");
		VM_BytecodeAction rc = EXECUTE_BYTECODE;
		j9object_t clazz = *(j9object_t*)_sp;
		j9object_t instance = NULL;
		buildInternalNativeStackFrame(REGISTER_ARGS);
	...
	{
		// printf("inside unsafe allocation");
		VM_BytecodeAction rc = EXECUTE_BYTECODE;
...
nativeOOM:
	...
	{
		// printf("came to return from constructor\n"); printing lot of times 
		VM_AtomicSupport::writeBarrier();
...
retry:
	...
	{
		// printf("came in for invokespecial logic\n");
		VM_BytecodeAction rc = GOTO_RUN_METHOD;
		U_16 index = *(U_16*)(_pc + 1);
		J9ConstantPool *ramConstantPool = J9_CP_FROM_METHOD(_literals);
...
foundITableCache:
	...
	{
		// printf("inside newLogic- line 7379\n");
		new_count = new_count +1;
		// ato
		printf("new count is %f\n",new_count);
retry:
		VM_BytecodeAction rc = EXECUTE_BYTECODE;
		U_16 index = *(U_16*)(_pc + 1);
		J9ConstantPool *ramConstantPool = J9_CP_FROM_METHOD(_literals);
	...
	{
		// printf("inside newImpl- line 7443\n");
		// new_count = new_count+1;
		// printf("new count is %f\n",new_count);
...
retry:
	...
	genericReturn(REGISTER_ARGS_LIST)
	{
		...
		if ('[' == sigData[sigLength - 2]) {
			...
			case 'V': {
				...
				if ((strlen("<init>") == J9UTF8_LENGTH(name)) && ('<' == J9UTF8_DATA(name)[0])) {
					printf("came into <init> signature");
					isConstructor = TRUE;
					VM_AtomicSupport::writeBarrier();
				}
				break;
		...
		if (J9ROMMETHOD_IS_OBJECT_CONSTRUCTOR(romMethod)) {
			...
			printf("It is a object constructor\n");
...
targetObjectConstructor:
	printf("came to targetObject Constructor");
